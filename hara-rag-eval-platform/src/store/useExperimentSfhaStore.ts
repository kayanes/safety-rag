...
export const useExperimentSfhaStore = create<ExperimentState>((set, get) => ({
    modelType: 'RAG-Model',
    itemDefinition: '',
    contextFile: null,
    results: [],
    ...
    runAnalysis: async (target: 'both' | 'base' | 'rag' = 'both') => {
        ...
        const fetchAnalysis = async (mode: 'rag' | 'base') => {
            const payload = {
                ...
                mode: mode,
                experiment_type: 'sfha', // Ensure we hit the SFHA prompts
            };
...
export const CHECKLIST_ITEMS = [
    {
        area: "SFHA 개념 이해",
        items: [
            { id: "sfha-understanding", title: "SFHA 정의의 정확성", desc: "SFHA의 목적과 개념이 체계(System) 수준에서 기준에 맞게 설명되었는가" }
        ]
    },
    {
        area: "시스템 기능 정의",
    ...
    },
    {
        area: "오작동 정의",
        items: [
            { id: "malfunction", title: "Malfunction 식별", desc: "시스템 기능과 관련된 체계 수준 오작동이 명확하게 식별되었는가" }
        ]
    },
    {
        area: "Hazard 정의",
    ...
    },
    {
        area: "Safety Goal 정의",
        items: [
            { id: "safety-goal", title: "Safety Goal 적절성", desc: "위험을 완화하기 위한 체계 관점 안전 목표가 명확히 제시되었는가" }
        ]
    },
    {
        area: "Safe State 정의",
